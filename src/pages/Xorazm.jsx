...
export default function XorazmContact() {
    ...
    // --- YANGILANGAN FUNKSIYA: MANZILNI QIDIRIB SAQLASH ---
    const saveNewLocation = async (e) => {
        ...
        try {
            ...
            const response = await fetch(
                `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(formData.name + " Xorazm")}&limit=1`
            );
    ...
    const handleEstablishLink = (location) => {
        const deviceId = "DEV-XORAZM-" + location.id.split('-')[1];
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input required autoFocus className="w-full bg-black/40 border border-emerald-500/20 rounded-xl px-4 py-3 text-sm focus:border-emerald-500 outline-none transition-all" 
                                    value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Masalan: Urganch Markaz" />
                            </div>
                            <div>
