...
export default function SurxondaryoSignalMapper() {
  ...
  const [savedLocations, setSavedLocations] = useState(() => {
    const saved = localStorage.getItem('surxondaryoLocations');
  ...
  const [linkedNode, setLinkedNode] = useState(() => {
    const saved = localStorage.getItem('surxondaryoLinkedNode');
  ...
  useEffect(() => {
    ...
    const center = [38.0, 67.5];
    
    const map = L.map(mapRef.current).setView(center, 8.5);
  ...
  const playRingtoneSound = useCallback(() => {
    ...
    console.log("📱 Rington: YANGI QURILMA SO'ROVI KELDI - SURXONDARYO");
  }, [createRingtoneSound]);
  ...
  useEffect(() => {
    localStorage.setItem('surxondaryoLocations', JSON.stringify(savedLocations));
  }, [savedLocations]);
  ...
  useEffect(() => {
    try {
      ...
      socketRef.current.onopen = () => {
        console.log("✅ WebSocket ulandi - SURXONDARYO (Railway)");
  ...
  const handleEstablishLink = (location) => {
    ...
    if (device.status === 'active') {
      ...
      localStorage.setItem('surxondaryoLinkedNode', JSON.stringify(newNode));
      if (mapInstanceRef.current) {
        mapInstanceRef.current.setView([location.lat, location.lng], 14);
      }
    }
  ...
  const handleStopAndDisconnect = () => {
    ...
    setLinkedNode(null);
    localStorage.removeItem('surxondaryoLinkedNode');
    stopAllSounds();
  };
