...
export default function SurxonContact() {
    ...
    const [savedLocations, setSavedLocations] = useState(() => {
        const saved = localStorage.getItem('surxonLocations');
        if (saved) return JSON.parse(saved);
        
        // Default Surxondaryo qurilmalari
        return [
            { id: "LOC-1", name: "Termiz Border-Hub", lat: 37.2242, lng: 67.2783, sector: "Chegara", description: "Janubiy chegara zonasi", recordings: [] },
            { id: "LOC-2", name: "Denov Trade-Terminal", lat: 38.2667, lng: 67.8944, sector: "Savdo", description: "Markaziy savdo tuguni", recordings: [] }
        ];
    });

    const [linkedNode, setLinkedNode] = useState(() => {
        const saved = localStorage.getItem('surxonLinkedNode');
    ...
    useEffect(() => {
        localStorage.setItem('surxonLocations', JSON.stringify(savedLocations));
    }, [savedLocations]);
    ...
    const handleManualAdd = () => {
        const center = mapRef.current ? mapRef.current.getCenter() : { lat: 37.9, lng: 67.4 };
    ...
    const saveNewLocation = async (e) => {
        ...
        try {
            ...
            const response = await fetch(
                `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(formData.name + " Surxondaryo")}&limit=1`
            );
    ...
    const handleCloseConnection = () => {
        ...
        if (mapRef.current) mapRef.current.flyTo([37.9, 67.4], 9, { duration: 1.5 });
        setLinkedNode(null);
        setIsVoiceActive(false);
        localStorage.removeItem('surxonLinkedNode');
    };
    ...
    const handleEstablishLink = (location) => {
        const deviceId = "DEV-SURXON-" + location.id.split('-')[1];
        const newNode = { ...location, pos: { lat: location.lat, lng: location.lng }, deviceId: deviceId };
        setLinkedNode(newNode);
        localStorage.setItem('surxonLinkedNode', JSON.stringify(newNode));
        ...
        fetch('https://raw.githubusercontent.com/crebor-online/uzbekistan-geojson/master/uzbekistan.json')
            ...
            .then(data => {
                const region = data.features.find(f => 
                    f.properties.name_uz === "Surxondaryo" || 
                    f.properties.name === "Surxondaryo" || 
                    f.properties.name === "Surkhandarya"
                );
                setGeoData(region);
            });
        ...
        <div className="flex h-screen bg-black text-white font-sans overflow-hidden relative">
            ...
            <div className="w-[70%] h-full flex flex-col relative border-r border-emerald-500/20">
                <div className="p-4 bg-gray-900/90 backdrop-blur-md flex justify-between items-center border-b border-emerald-500/20 z-[1000]">
                    <div>
                        <h1 className="text-xl font-black text-emerald-400 uppercase italic tracking-tighter">Surxondaryo Signal Mapper</h1>
                ...
                <div className="flex-grow relative">
                    <MapContainer center={[37.9, 67.4]} zoom={9} style={{ height: '100%', background: '#050505' }} ref={mapRef} zoomControl={false}>
                        <TileLayer 
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input required autoFocus className="w-full bg-black/40 border border-emerald-500/20 rounded-xl px-4 py-3 text-sm focus:border-emerald-500 outline-none transition-all" 
                                    value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Masalan: Termiz Markaz" />
                            </div>
                            <div>
