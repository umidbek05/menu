...
const jizzaxSectors = [
    { id: 1, name: "Jizzax Central-Node", lat: 40.1158, lng: 67.8422, address: "Jizzax sh., Sh.Rashidov shoh ko'chasi" },
    { id: 2, name: "Zomin Alpine-Post", lat: 39.9606, lng: 68.3958, address: "Zomin tumani, Milliy bog' hududi" },
    { id: 3, name: "Forish Desert-Unit", lat: 40.5667, lng: 67.1167, address: "Forish tumani, Aydarkul sektori" },
    { id: 4, name: "G'allaorol Transit-Hub", lat: 40.0267, lng: 67.5856, address: "G'allaorol, M-39 xalqaro trassasi" },
    { id: 5, name: "Paxtakor Agro-Sector", lat: 40.3167, lng: 67.9500, address: "Paxtakor tumani, Markaziy stansiya" },
    { id: 6, name: "Do'stlik Relay-Post", lat: 40.5250, lng: 68.0333, address: "Do'stlik tumani, Shimoliy aloqa nuqtasi" },
    { id: 7, name: "Baxmal Mountain-Link", lat: 39.8167, lng: 67.9000, address: "Baxmal, Janubiy tizmalar" }
];
...
export default function JizzaxContact() {
    ...
    const [savedLocations, setSavedLocations] = useState(() => {
        const saved = localStorage.getItem('jizzaxLocations');
        if (saved) return JSON.parse(saved);
        
        return jizzaxSectors.map(device => ({
            ...device,
            sector: device.name.split(' ')[0] || "Jizzax",
    ...
    const [linkedNode, setLinkedNode] = useState(() => {
        const saved = localStorage.getItem('jizzaxLinkedNode');
    ...
    useEffect(() => {
        localStorage.setItem('jizzaxLocations', JSON.stringify(savedLocations));
    }, [savedLocations]);
    ...
    const handleManualAdd = () => {
        const center = mapRef.current ? mapRef.current.getCenter() : { lat: 40.4, lng: 67.8 };
    ...
    const saveNewLocation = async (e) => {
        ...
        try {
            const response = await fetch(
                `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(formData.name + " Jizzax")}&limit=1`
            );
    ...
    const handleCloseConnection = () => {
        ...
        if (mapRef.current) mapRef.current.flyTo([40.4, 67.8], 9, { duration: 1.5 });
        setLinkedNode(null);
        setIsVoiceActive(false);
        localStorage.removeItem('jizzaxLinkedNode');
    };

    const handleEstablishLink = (location) => {
        const deviceId = "DEV-JIZZAX-" + location.id.split('-')[1];
        const newNode = { ...location, pos: { lat: location.lat, lng: location.lng }, deviceId: deviceId };
        setLinkedNode(newNode);
        localStorage.setItem('jizzaxLinkedNode', JSON.stringify(newNode));
        ...
        fetch('https://raw.githubusercontent.com/crebor-online/uzbekistan-geojson/master/uzbekistan.json')
            ...
            .then(data => {
                const region = data.features.find(f => 
                    f.properties.name_uz === "Jizzax" || 
                    f.properties.name === "Jizzax" || 
                    f.properties.name === "Jizzakh"
                );
                setGeoData(region);
            });
        ...
        <div className="flex h-screen bg-black text-white font-sans overflow-hidden relative">
            ...
            <div className="w-[70%] h-full flex flex-col relative border-r border-emerald-500/20">
                <div className="p-4 bg-gray-900/90 backdrop-blur-md flex justify-between items-center border-b border-emerald-500/20 z-[1000]">
                    <div>
                        <h1 className="text-xl font-black text-emerald-400 uppercase italic tracking-tighter">Jizzax Signal Mapper</h1>
            ...
            <div className="w-[30%] bg-[#080d0b] p-6 flex flex-col gap-6">
                <div className="flex justify-between items-center border-b border-emerald-500/20 pb-4">
                    <div className="flex flex-col">
                        <h3 className="text-emerald-500 text-[10px] font-black tracking-[0.3em] uppercase italic">Jizzax Manzillar</h3>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input required autoFocus className="w-full bg-black/40 border border-emerald-500/20 rounded-xl px-4 py-3 text-sm focus:border-emerald-500 outline-none transition-all" 
                                    value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Masalan: Jizzax Markaz" />
                            </div>
                            <div>
