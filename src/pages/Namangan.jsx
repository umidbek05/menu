...
export default function NamanganContact() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      console.log("🕐 Vaqt yangilandi:", new Date().toLocaleTimeString());
    }, 1000);
  ...
  useEffect(() => {
    ...
    savedLocations.forEach(loc => {
      console.log(`📍 Marker qo'shilmoqda: ${loc.sector} (${loc.lat}, ${loc.lng})`);
      ...
      marker.on('click', () => {
        console.log(`🖱️ Marker bosildi: ${loc.sector}`);
    ...
    map.on('click', (e) => {
      console.log(`🗺️ Xarita bosildi: (${e.latlng.lat}, ${e.latlng.lng})`);
    ...
    return () => {
      console.log("🗺️ Xarita o'chirilmoqda");
      map.remove();
  ...
  useEffect(() => {
    ...
    console.log("🔄 Markerlarni yangilash, jami:", savedLocations.length, "ta");
    ...
    console.log("✅ Markerlarni yangilash tugadi");
  }, [savedLocations]);
  ...
  useEffect(() => {
    ...
    console.log("🔍 Qidiruv natijasi markazlashtirilmoqda:", searchResult.displayName);
    const map = mapInstanceRef.current;
  ...
  const createRingtoneSound = useCallback(() => {
    console.log("🔊 Ringtone ovoz yaratilmoqda");
  ...
  const playRingtoneSound = useCallback(() => {
    console.log("🔊 Rington chalinmoqda");
    const audioBuffer = createRingtoneSound();
  ...
  const startMicrophone = async () => {
    if (isMicActiveRef.current) {
      console.log("🎤 Mikrofon allaqachon faol");
      return;
    }

    try {
      console.log("🎤 Mikrofon ishga tushirilmoqda...");
      ...
      processor.onaudioprocess = (e) => {
        if (!socketRef.current || socketRef.current.readyState !== WebSocket.OPEN || !linkedNode || !isMicActiveRef.current) {
          return;
        }
      ...
      console.log("✅ Mikrofon muvaffaqiyatli ishga tushirildi");

    } catch (error) {
      console.error("❌ Mikrofonni ishga tushirishda xato:", error);
      isMicActiveRef.current = false;
      throw error;
    }
  };

  // Audio buffer'ni ijro etish
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      try {
        ...
        ws.onopen = () => {
          console.log("✅ WebSocket ulandi - NAMANGAN (Railway)");
        ...
        ws.onmessage = (event) => {
          try {
            if (typeof event.data === 'string') {
              ...
              console.log("📩 WebSocket xabar keldi:", data.type, data);
              
              if (data.type === 'list') {
                console.log("📋 Qurilmalar ro'yxati:", data.devices?.length || 0, "ta");
                setEspDevices(data.devices || []);
              } 
              else if (data.type === 'new_pending_device') {
                console.log("🆕 Yangi pending qurilma keldi:", data.device);
                playRingtoneSound();
                setEspDevices(prev => {
                  if (!prev.some(d => d.id === data.device.id)) {
                    console.log("➕ Yangi qurilma qo'shilmoqda:", data.device.id);
                ...
                if (linkedNode && linkedNode.deviceId === data.deviceId) {
              ...
              if (linkedNode && audioCtxRef.current && !isMicActiveRef.current) {
                const audioData = new Int16Array(event.data);
                const audioBuffer = audioCtxRef.current.createBuffer(1, audioData.length, 16000);
                const channelData = audioBuffer.getChannelData(0);
        ...
        ws.onclose = () => {
          console.log("🔌 WebSocket uzildi. 5 soniyadan keyin qayta ulanish...");
          isMicActiveRef.current = false;
          setEspDevices(prev => prev.map(d => ({ ...d, status: 'disconnected' })));
        ...
        console.error("❌ WebSocket ulanishda xato:", e);
        setTimeout(connectWebSocket, 5000);
      }
    };
    ...
    return () => {
      console.log("🔌 WebSocket yopilmoqda...");
  ...
  const isDeviceIdUnique = (deviceId, currentLocationId = null) => {
    ...
    if (exists) {
      console.log(`⚠️ Device ID ${deviceId} allaqachon mavjud`);
    } else {
      console.log(`✅ Device ID ${deviceId} unique`);
    }
  ...
  const handleDeviceIdChange = (e) => {
    ...
    console.log(`📝 Device ID o'zgartirilmoqda: "${newValue}"`);
  ...
  const handleSearchByLink = async () => {
    ...
    console.log("🔍 Qidiruv boshlanmoqda:", address);
    setIsSearching(true);
    setSearchError('');
    ...
    try {
      ...
      if (address.includes('goo.gl/maps') || 
        ...
        if (coords) {
          lat = coords.lat;
          lng = coords.lng;
          console.log(`📍 Havoladan koordinatalar topildi: ${lat}, ${lng}`);
        ...
        console.log(`📍 Koordinatalar: ${lat}, ${lng}`);
        ...
        if (data && data.length > 0) {
          ...
          console.log(`📍 Manzil topildi: ${displayName} (${lat}, ${lng})`);
        } else {
      ...
      console.log("✅ Qidiruv muvaffaqiyatli yakunlandi");
  ...
  const handleSaveSearchResult = () => {
    ...
    console.log("💾 Qidiruv natijasini saqlash:", searchResult.displayName);
    setEditingId(null);
  ...
  const handleClearSearch = () => {
    console.log("🗑️ Qidiruv tozalandi");
    setSearchResult(null);
    setSearchLink('');
    setSearchError('');
  ...
  const handleMarkerClick = (location) => {
    console.log(`✏️ Marker tahrirlash: ${location.sector} (${location.id})`);
  ...
  const handleMapPick = async (latlng) => {
    console.log(`➕ Yangi nuqta qo'shilmoqda: (${latlng.lat}, ${latlng.lng})`);
    setEditingId(null);
    ...
    try {
      ...
      if (data && data.display_name) {
        console.log(`📍 Manzil topildi: ${data.display_name}`);
        setFormData((prev) => ({
          ...prev,
  ...
  const saveNewLocation = (e) => {
    ...
    if (editingId) {
      console.log(`✏️ Manzil tahrirlanmoqda: ${editingId}`);
      ...
      console.log(`➕ Yangi manzil qo'shilmoqda: ${formData.name}`);
      const newLocation = {
  ...
  const handleDeleteLocation = (id) => {
    ...
    console.log(`🗑️ Manzil o'chirilmoqda: ${location?.sector} (${id})`);
    
    if (window.confirm("Ushbu manzil va unga tegishli barcha yozuvlar o'chib ketadi. Rozimisiz?")) {
      setSavedLocations(prev => prev.filter(loc => loc.id !== id));
      if (linkedNode?.id === id) {
        console.log("🔗 Bog'langan node ham o'chirilmoqda");
        handleStopAndDisconnect();
      }
  ...
  const handleEstablishLink = (location) => {
    console.log(`🔗 Bog'lanish o'rnatilmoqda: ${location.sector} (${location.deviceId})`);
    const device = espDevices.find(d => d.id === location.deviceId);
    
    if (!device) {
      console.error(`❌ Qurilma topilmadi. ID: ${location.deviceId || 'Noma\'lum'}`);
      alert(`❌ Qurilma topilmadi. ID: ${location.deviceId || 'Noma\'lum'}`);
      return;
    }

    if (device.status === 'pending') {
      console.log(`⏳ Ruxsat so'rovi yuborilmoqda: ${device.id}`);
    ...
    if (device.status === 'active') {
      console.log(`✅ Bog'lanish o'rnatildi: ${location.sector}`);
      const newNode = { 
        ...location, 
        deviceId: device.id 
      };
  ...
  const handleStopAndDisconnect = () => {
    console.log("🔌 Bog'lanish uzilmoqda...");
    isMicActiveRef.current = false;
    ...
    localStorage.removeItem('namanganLinkedNode');
    stopAllSounds();
    console.log("✅ Bog'lanish uzildi");
  };

  // Ovozli aloqa boshlash
  const startAudio = async () => {
    console.log("🎙️ Ovozli aloqa boshlanmoqda...");
    try {
      await startMicrophone();
      setIsVoiceActive(true);
      console.log("✅ Ovozli aloqa boshlandi");
    } catch (error) {
  ...
  const stopAudio = () => {
    console.log("🔇 Ovozli aloqa to'xtatilmoqda...");
    isMicActiveRef.current = false;
    ...
    setIsVoiceActive(false);
    console.log("✅ Ovozli aloqa to'xtatildi");
  };

  // Yozuvni o'chirish
  const deleteRecording = (locationId, recordingId) => {
    console.log(`🗑️ Yozuv o'chirilmoqda: ${recordingId}`);
  ...
  const handleAuthorize = (deviceId) => {
    console.log(`📤 Avtorizatsiya so'rovi yuborilmoqda: ${deviceId}`);
    if (socketRef.current && socketRef.current.readyState === WebSocket.OPEN) {
  ...
  return (
    <div className="flex h-screen bg-black text-white font-sans overflow-hidden relative">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Masalan: Namangan-001"
                />
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Masalan: ESP32-001 yoki NM-01"
                />
                {deviceIdError ? (
      ...
      {/* ARXIV MODALI - XORAZM VERSIYASIDAGI KABI */}
{showArchive && (
  <div className="absolute inset-0 z-[3000] bg-black/80 backdrop-blur-xl flex items-center justify-center p-10">
                  ...
                  <div key={rec.id} className="bg-emerald-500/5 p-3 rounded-2xl border border-emerald-500/5 group">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-[8px] text-gray-400 font-mono">
                        {rec.timestamp ? new Date(rec.timestamp).toLocaleString() : rec.time}
                      </span>
                      <button 
                    ...
                    {rec.data ? (
                      ...
                      <p className="text-[10px] text-gray-500">Audio ma'lumot mavjud emas</p>
                    )}
                  </div>
                ))}
              </div>
