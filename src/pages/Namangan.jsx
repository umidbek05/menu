...
export default function NamanganContact() {
    ...
    const playRingtoneSound = () => {
        ...
        console.log("📱 Mayin rington: YANGI QURILMA SO'ROVI KELDI - NAMANGAN");
    };
    ...
    const handleManualAdd = () => {
        setEditingId(null);
        const center = mapRef.current ? mapRef.current.getCenter() : { lat: 41.0, lng: 71.5 };
    ...
    const handleCloseConnection = () => {
        ...
        if (mapRef.current) mapRef.current.flyTo([41.0, 71.5], 10, { duration: 1.5 });
        setLinkedNode(null);
        setIsVoiceActive(false);
        ...
        fetch('https://raw.githubusercontent.com/crebor-online/uzbekistan-geojson/master/uzbekistan.json')
            ...
            .then(data => {
                const region = data.features.find(f => 
                    f.properties.name_uz === "Namangan" || 
                    f.properties.name === "Namangan"
                );
                setGeoData(region);
            })
        ...
        <div className="flex h-screen bg-black text-white font-sans overflow-hidden relative">
            ...
            <div className="w-[70%] h-full flex flex-col relative border-r border-emerald-500/20">
                <div className="p-4 bg-gray-900/90 backdrop-blur-md border-b border-emerald-500/20 z-[1000]">
                    {/* Navbar qismi - XORAZM/NAVOIY BILAN BIR XIL */}
                ...
                <div className="flex-grow relative">
                    <MapContainer center={[41.0, 71.5]} zoom={10} style={{ height: '100%', background: '#050505' }} ref={mapRef} zoomControl={false}>
