...
export default function NamanganContact() {
    ...
    const handleManualAdd = () => {
        const center = mapRef.current ? mapRef.current.getCenter() : { lat: 41.0, lng: 71.45 };
    ...
    const saveNewLocation = async (e) => {
        ...
        try {
            const response = await fetch(
                `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(formData.name + " Namangan")}&limit=1`
            );
    ...
    const handleCloseConnection = () => {
        ...
        if (mapRef.current) mapRef.current.flyTo([41.0, 71.45], 10, { duration: 1.5 });
        setLinkedNode(null);
        ...
        <div className="flex h-screen bg-black text-white font-sans overflow-hidden relative">
            ...
            <div className="w-[70%] h-full flex flex-col relative border-r border-emerald-500/20">
                <div className="p-4 bg-gray-900/90 backdrop-blur-md flex justify-between items-center border-b border-emerald-500/20 z-[1000]">
                    <div>
                        <h1 className="text-xl font-black text-emerald-400 uppercase italic tracking-tighter">Namangan Signal Mapper</h1>
                ...
                <div className="flex-grow relative">
                    <MapContainer center={[41.0, 71.45]} zoom={10} style={{ height: '100%', background: '#050505' }} ref={mapRef} zoomControl={false}>
                        <TileLayer 
            ...
            <div className="w-[30%] bg-[#080d0b] p-6 flex flex-col gap-6">
                <div className="flex justify-between items-center border-b border-emerald-500/20 pb-4">
                    <div className="flex flex-col">
                        <h3 className="text-emerald-500 text-[10px] font-black tracking-[0.3em] uppercase italic">Namangan Manzillar</h3>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input required autoFocus className="w-full bg-black/40 border border-emerald-500/20 rounded-xl px-4 py-3 text-sm focus:border-emerald-500 outline-none transition-all" 
                                    value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Masalan: Namangan Markaz" />
                            </div>
                            <div>
