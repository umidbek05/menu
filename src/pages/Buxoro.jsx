...
const bukharaNodes = [
    { id: 1, name: "Bukhara Citadel-Hub", lat: 39.7747, lng: 64.4286, address: "Buxoro sh., Ark qo'rg'oni hududi" },
    { id: 2, name: "Gazli Energy-Post", lat: 40.1311, lng: 63.4561, address: "Gazli shahri, Markaziy gaz terminali" },
    { id: 3, name: "Kogon Railway-Sector", lat: 39.7167, lng: 64.5500, address: "Kogon sh., Logistika markazi" },
    { id: 4, name: "Qorako'l Border-Node", lat: 39.5000, lng: 63.8500, address: "Qorako'l, Janubiy nazorat nuqtasi" },
    { id: 5, name: "G'ijduvon Industrial-Unit", lat: 40.1000, lng: 64.6667, address: "G'ijduvon, Hunarmandlar zonasi" },
    { id: 6, name: "Qorovulbozor Refinery", lat: 39.5000, lng: 64.8167, address: "Qorovulbozor, Neftni qayta ishlash zavodi" },
    { id: 7, name: "Shofirkon Relay-Post", lat: 40.1167, lng: 64.5000, address: "Shofirkon shimoliy aloqa minorasi" }
];
...
export default function BukharaContact() {
    ...
    const [savedLocations, setSavedLocations] = useState(() => {
        const saved = localStorage.getItem('bukharaLocations');
        if (saved) return JSON.parse(saved);
        
        return bukharaNodes.map(device => ({
            ...device,
            sector: device.name.split(' ')[0] || "Buxoro",
    ...
    const [linkedNode, setLinkedNode] = useState(() => {
        const saved = localStorage.getItem('bukharaLinkedNode');
    ...
    useEffect(() => {
        localStorage.setItem('bukharaLocations', JSON.stringify(savedLocations));
    }, [savedLocations]);
    ...
    const handleManualAdd = () => {
        const center = mapRef.current ? mapRef.current.getCenter() : { lat: 39.85, lng: 64.3 };
    ...
    const saveNewLocation = async (e) => {
        ...
        try {
            const response = await fetch(
                `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(formData.name + " Buxoro")}&limit=1`
            );
    ...
    const handleCloseConnection = () => {
        ...
        if (mapRef.current) mapRef.current.flyTo([39.85, 64.3], 9, { duration: 1.5 });
        setLinkedNode(null);
        setIsVoiceActive(false);
        localStorage.removeItem('bukharaLinkedNode');
    };

    const handleEstablishLink = (location) => {
        const deviceId = "DEV-BUKHARA-" + location.id.split('-')[1];
        const newNode = { ...location, pos: { lat: location.lat, lng: location.lng }, deviceId: deviceId };
        setLinkedNode(newNode);
        localStorage.setItem('bukharaLinkedNode', JSON.stringify(newNode));
        ...
        fetch('https://raw.githubusercontent.com/crebor-online/uzbekistan-geojson/master/uzbekistan.json')
            ...
            .then(data => {
                const region = data.features.find(f => 
                    f.properties.name_uz === "Buxoro" || 
                    f.properties.name === "Buxoro" || 
                    f.properties.name === "Bukhara"
                );
                setGeoData(region);
            });
        ...
        <div className="flex h-screen bg-black text-white font-sans overflow-hidden relative">
            ...
            <div className="w-[70%] h-full flex flex-col relative border-r border-emerald-500/20">
                <div className="p-4 bg-gray-900/90 backdrop-blur-md flex justify-between items-center border-b border-emerald-500/20 z-[1000]">
                    <div>
                        <h1 className="text-xl font-black text-emerald-400 uppercase italic tracking-tighter">Buxoro Signal Mapper</h1>
            ...
            <div className="w-[30%] bg-[#080d0b] p-6 flex flex-col gap-6">
                <div className="flex justify-between items-center border-b border-emerald-500/20 pb-4">
                    <div className="flex flex-col">
                        <h3 className="text-emerald-500 text-[10px] font-black tracking-[0.3em] uppercase italic">Buxoro Manzillar</h3>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input required autoFocus className="w-full bg-black/40 border border-emerald-500/20 rounded-xl px-4 py-3 text-sm focus:border-emerald-500 outline-none transition-all" 
                                    value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Masalan: Buxoro Markaz" />
                            </div>
                            <div>
