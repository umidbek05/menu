...
// Toshkent qurilmalari ma'lumotlari - DEFAULT YO'Q
const toshkentDevices = [];  // Bo'sh array - hech qanday default qurilma yo'q
...
export default function ToshkentManager() {
  ...
  const [savedLocations, setSavedLocations] = useState(() => {
    ...
    // Agar localStorage bo'sh bo'lsa, Toshkent qurilmalarini default qilib qo'yish
    return toshkentDevices.map(device => ({
      id: `LOC-${device.id}`,
      name: device.name,
      lat: device.lat,
      lng: device.lng,
  ...
  const playRingtoneSound = useCallback(() => {
    ...
    console.log("📱 Rington: YANGI QURILMA SO'ROVI KELDI - TOSHKENT");
  }, [createRingtoneSound]);
  ...
  useEffect(() => {
    try {
      ...
      socketRef.current.onopen = () => {
        console.log("✅ WebSocket ulandi - TOSHKENT (Railway)");
  ...
  // ✅ TUZATILGAN: Bog'lanish o'rnatish - Xorazm bilan bir xil
  ...
  // ✅ TUZATILGAN: Bog'lanishni uzish - Xorazm bilan bir xil
        ...
        <div className="p-4 bg-gray-900/90 backdrop-blur-md border-b border-emerald-500/20 z-[1000]">
          <div className="flex items-center justify-between gap-4">
            <h1 className="text-xl font-black text-emerald-400 uppercase italic tracking-tighter flex items-center gap-2 whitespace-nowrap">
              <Map className="w-5 h-5" /> Toshkent Signal Mapper
            </h1>
        ...
        <div className="absolute inset-0 z-[3000] bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
          <form onSubmit={saveNewLocation} className="bg-[#0c1410] border border-emerald-500/40 p-8 rounded-[32px] w-full max-w-md shadow-2xl scale-in">
            ...
            <div className="space-y-4">
              ...
              {/* Qurilma ID (ESP dan keladigan ID) */}
              <div>
                ...
                <input
                  ...
                  placeholder="Masalan: ESP32-001 yoki Toshkent-01"
                />
                {deviceIdError ? (
