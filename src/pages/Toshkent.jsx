...
export default function ToshkentManager() {
    ...
    const [savedLocations, setSavedLocations] = useState(() => {
        const saved = localStorage.getItem('toshkentLocations');
        if (saved) return JSON.parse(saved);
        
        // Default Toshkent qurilmalari
        return [
            { id: "LOC-1", name: "Chirchiq Terminal-04", lat: 41.4689, lng: 69.5822, sector: "Sanoat", description: "ESP32_TOSH_01", recordings: [] },
            { id: "LOC-2", name: "Angren Energy-Hub", lat: 41.0167, lng: 70.1433, sector: "Energetika", description: "ESP32_TOSH_02", recordings: [] }
        ];
    });

    const [linkedNode, setLinkedNode] = useState(() => {
        const saved = localStorage.getItem('toshkentLinkedNode');
    ...
    useEffect(() => {
        localStorage.setItem('toshkentLocations', JSON.stringify(savedLocations));
    }, [savedLocations]);
    ...
    const handleManualAdd = () => {
        const center = mapRef.current ? mapRef.current.getCenter() : { lat: 41.31, lng: 69.28 };
    ...
    const saveNewLocation = async (e) => {
        ...
        try {
            ...
            const response = await fetch(
                `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(formData.name + " Toshkent")}&limit=1`
            );
    ...
    const handleCloseConnection = () => {
        ...
        if (mapRef.current) mapRef.current.flyTo([41.31, 69.28], 10, { duration: 1.5 });
        setLinkedNode(null);
        setIsVoiceActive(false);
        localStorage.removeItem('toshkentLinkedNode');
    };
    ...
    const handleEstablishLink = (location) => {
        const deviceId = "DEV-TOSHKENT-" + location.id.split('-')[1];
        const newNode = { ...location, pos: { lat: location.lat, lng: location.lng }, deviceId: deviceId };
        setLinkedNode(newNode);
        localStorage.setItem('toshkentLinkedNode', JSON.stringify(newNode));
        ...
        fetch('https://raw.githubusercontent.com/crebor-online/uzbekistan-geojson/master/uzbekistan.json')
            ...
            .then(data => {
                const region = data.features.find(f => 
                    f.properties.name_uz === "Toshkent" || 
                    f.properties.name === "Toshkent" || 
                    f.properties.name === "Tashkent"
                );
                setGeoData(region);
            });
        ...
        <div className="flex h-screen bg-black text-white font-sans overflow-hidden relative">
            ...
            <div className="w-[70%] h-full flex flex-col relative border-r border-emerald-500/20">
                <div className="p-4 bg-gray-900/90 backdrop-blur-md flex justify-between items-center border-b border-emerald-500/20 z-[1000]">
                    <div>
                        <h1 className="text-xl font-black text-emerald-400 uppercase italic tracking-tighter">Toshkent Signal Mapper</h1>
                ...
                <div className="flex-grow relative">
                    <MapContainer center={[41.31, 69.28]} zoom={10} style={{ height: '100%', background: '#050505' }} ref={mapRef} zoomControl={false}>
                        <TileLayer 
            ...
            <div className="w-[30%] bg-[#080d0b] p-6 flex flex-col gap-6">
                <div className="flex justify-between items-center border-b border-emerald-500/20 pb-4">
                    <div className="flex flex-col">
                        <h3 className="text-emerald-500 text-[10px] font-black tracking-[0.3em] uppercase italic">Saqlangan Manzillar</h3>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input required autoFocus className="w-full bg-black/40 border border-emerald-500/20 rounded-xl px-4 py-3 text-sm focus:border-emerald-500 outline-none transition-all" 
                                    value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Masalan: Toshkent Markaz" />
                            </div>
                            <div>
